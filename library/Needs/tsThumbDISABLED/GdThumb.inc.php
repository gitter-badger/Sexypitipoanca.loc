...
class GdThumb extends ThumbBase
{
	...
	/**
	 * Resizes an image to be no larger than $maxHeight with color fill
	 * 
	 * If either param is set to zero, then that dimension will not be considered as a part of the resize.
	 * Additionally, if $this->options['resizeUp'] is set to true (false by default), then this function will
	 * also scale the image up to the maximum dimensions provided.
	 * 
	 * @param int $newWidth The maximum width of the new image in pixels
	 * @param int $newHeight The maximum height of the new image in pixels
	 * @param int $x The x coordonant of the selection in pixels
	 * @param int $y The y coordonant of the selection in pixels
	 * @param int $w The width of the selection in pixels
	 * @param int $h The height of the selection in pixels
	 * @return GdThumb
	 */
	public function tsResizeWithFill($newWidth, $newHeight, $fillColor)
	{
		$oldWidth = $this->currentDimensions['width'];
		$oldHeight = $this->currentDimensions['height'];
		
		$rgb = $this->tsHex2Dec($fillColor);
		...
		if(function_exists('imagecreatetruecolor')){
			$temp = imagecreatetruecolor($resizeDim, $resizeDim);
		}else{
			$temp = imagecreate($resizeDim, $resizeDim);
		}

		$color = imagecolorallocate($temp, $rgb['r'], $rgb['g'], $rgb['b']);
		imagefill($temp, 0, 0, $color);
		
		if($oldWidth > $oldHeight){
			$destX = 0;
			$destY = round($oldWidth-$oldHeight)/2;
		}else{
			$destX = round($oldHeight-$oldWidth)/2;
			$destY = 0;
		}

		$this->preserveAlpha();
		imagecopymerge($temp, $this->oldImage, $destX, $destY , 0 , 0 ,$oldWidth , $oldHeight , 100 );
		
		if(function_exists('imagecreatetruecolor')){
			$this->workingImage = imagecreatetruecolor($newWidth, $newWidth);
		}else{
			$this->workingImage = imagecreate($newWidth, $newWidth);
		}
		imagecopyresampled(
			$this->workingImage,
			$temp,
	 ...
	 * @return GdThumb
	 */
	public function tsCropFromCenter ($newWidth, $newHeight, $x = 0, $y = 0, $w = 0, $h = 0)
	{
		if(function_exists('imagecreatetruecolor')){
		...
		// update all the variables and resources to be correct
		$this->oldImage 					= $this->workingImage;
		$this->currentDimensions['width'] 	= $w;
		$this->currentDimensions['height'] 	= $h;
